...
import br.com.setebit.sgr.dto.Summary;
...
import br.com.setebit.sgr.service.TicketService;
...
public class TicketController {
	...
	public ResponseEntity<Response<Ticket>> update(HttpServletRequest request, @RequestBody Ticket ticket,
		...
		try {
			...
			Optional<Ticket> ticketCurrentOptional = ticketService.findById(ticket.getId().toString());
	...
	public ResponseEntity<Response<Ticket>> findById(@PathVariable("id") String id) {
		...
		Iterable<ChangeStatus> changesCurrent =  ticketService.listChangeStatus(ticket.getId().toString());
    ...
    public  ResponseEntity<Response<Page<Ticket>>> findAll(HttpServletRequest request, @PathVariable int page, @PathVariable int count) {
		...
		if(userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
			...
			tickets = ticketService.findByCurrentUser(page, count, userRequest.getId().toString());
		}
    ...
    public  ResponseEntity<Response<Page<Ticket>>> findByParams(HttpServletRequest request, 
		...
		if(number > 0) {
			...
			if(userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
				if(assigned) {
					tickets = ticketService.findByParametersAndAssignedUser(page, count, title, status, priority, userRequest.getId().toString());
				} else {
					tickets = ticketService.findByParameters(page, count, title, status, priority);
				}
			} else if(userRequest.getProfile().equals(ProfileEnum.ROLE_CUSTOMER)) {
				tickets = ticketService.findByParametersAndCurrentUser(page, count, title, status, priority, userRequest.getId().toString());
			}
		}
		response.setData(tickets);
		return ResponseEntity.ok(response);
